...
if tables.n > 0:
    ...
    def extract_one_split(split):
        ...
        i = 0
        while starting_size == "":
            starting_size = split.iloc[1, i]
            i += 1

        if starting_size == "XXS":
            preferred_column_list = column_list1
            preferred_size_list = size_list1
        elif starting_size == "S":
            preferred_column_list = column_list2
            preferred_size_list = size_list2

        for i in range(0, len(split), 2):
            if split.iloc[i, preferred_column_list.index("Color Code")] != "":
                item = split[i : i + 2]
                extract_one_item(item, preferred_column_list, preferred_size_list)
